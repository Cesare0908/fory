document.addEventListener("DOMContentLoaded", () => {
    ...
    const tablaProductosMasVendidos = new gridjs.Grid({
        ...
        resizable: true,
        server: {
            url: "http://localhost/fory-final/controladores/controladorDash.php?ope=productosMasVendidos",
    ...
    const tablaClientesPrincipales = new gridjs.Grid({
        ...
        resizable: true,
        server: {
            url: "http://localhost/fory-final/controladores/controladorDash.php?ope=clientesPrincipales",
    ...
    const tablaAlertasStockBajo = new gridjs.Grid({
        ...
        resizable: true,
        server: {
            url: "http://localhost/fory-final/controladores/controladorDash.php?ope=alertasStockBajo",
            then: (data) => data.resultados.map((producto) => [
                producto.nombre_producto || 'N/A',
                producto.nombre_categoria || 'N/A',
                producto.stock || 0,
    ...
    function cargarDatosDashboard(queryParams = '') {
        const url = queryParams 
            ? `http://localhost/fory-final/controladores/controladorDash.php?ope=datosDashboard&${queryParams}`
            : 'http://localhost/fory-final/controladores/controladorDash.php?ope=datosDashboard';
            ...
            .then(data => {
                if (data.success) {
                    ...
                    pedidosPorEstadoChart = new Chart(document.getElementById('pedidosPorEstadoChart'), {
                        type: 'pie', 
                        data: { 
                            labels: ['Pendiente', 'En camino', 'En proceso', 'Entregado', 'Cancelado'], 
                            datasets: [{ 
                                data: [
                                    datos.pedidosPorEstado.pendiente, 
                                    datos.pedidosPorEstado.camino, 
                                    datos.pedidosPorEstado.proceso,
